    ...
    def conditional_distribution(self, xti, T=1, reverse=False):
        ...
        if not reverse:
            ...
            for i in range(T):
                ...
                _mu = -self.velocity(_mu) * self.dt + _mu
                _var = torch.bmm(torch.bmm(Ad, _var), Ad) + self.var * self.dt
        return tdist.MultivariateNormal(_mu, _var, validate_args=False)
...
class LimitCycleDynamicModel(nn.Module):
    ...
    def generate_trj_hr(self, xti, x_0, T=1, reverse=False):
        mu = self.generate_trj(xti, T, reverse=reverse)
        # mu_0 (known)
        # mu_1 (unknown)
        len_known = x_0.shape[0]
        mu_0 = mu[:len_known]
        mu_1 = mu[len_known:]
        x_1 = mu_1 + sigma_10 * (sigma_00^-1)*(x_0 - mu_0)
