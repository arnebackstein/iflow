...
class HRLinearLimitCycle(LimitCycleDynamicModel):
    ''' The Dynamic model is composed of two parts. A limit '''
    def __init__(self, dim_h, dim_r, device=None, dt = 0.001, requires_grad=True, T_to_stable=1., T_period= 1.):
        dim = dim_h+dim_r
        ...
        len_tril = int(dim*(dim-1)/2)
        self.tril_var = nn.Parameter(torch.zeros(len_tril)).to(device).requires_grad_(requires_grad)
    ...
    def var(self):
        ...
        L[np.tril_indices(self.dim, k=-1)] = self.tril_var
        L += torch.diag(torch.ones(self.dim))

        D = torch.diag(torch.exp(self.log_var))
